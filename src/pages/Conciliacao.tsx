...
export default function Conciliacao() {
  ...
  const openMatch = async (e: { id: string; descricao: string; valor: number }) => {
    ...
    try {
      const c = await fetchCandidates(e.id);
      setCandidates(c);
    } finally {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader
        ...
        description="Confronto entre extratos bancários importados e lançamentos do fluxo de caixa"
      >
      ...
      <Dialog open={!!matchEntry} onOpenChange={(o) => !o && setMatchEntry(null)}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>Conciliar com lançamento</DialogTitle>
          </DialogHeader>
          {matchEntry && (
