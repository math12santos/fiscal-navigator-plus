...
export default function BackofficeOnboarding() {
    ...
    async function fetch() {
      ...
      setData(
        (progressData as any[]).map((p: any) => ({
          ...p,
          org_name: orgMap.get(p.organization_id) || "—",
        }))
      );
      setLoading(false);
    }
  ...
  const stats = {
    total: data.length,
    em_andamento: data.filter((d) => d.status === "em_andamento").length,
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Acompanhe a implantação das empresas</p>
        </div>
      </div>
                ...
                return (
                  <>
                    <TableRow key={item.id} className="cursor-pointer" onClick={() => setExpandedId(isExpanded ? null : item.id)}>
                      ...
                      <TableCell>
                        {item.maturity_level ? (
                          <Badge variant="outline">Nível {item.maturity_level}</Badge>
                        ) : (
                          ...
                          <div className="py-3 px-2">
                            ...
                            <div className="flex flex-wrap gap-2">
                              {Array.from({ length: 10 }, (_, i) => i + 1).map((step) => (
                                <Badge
                                  key={step}
                                  variant={item.completed_steps.includes(step) ? "default" : "outline"}
                                  className="text-xs"
                                >
