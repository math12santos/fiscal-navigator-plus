...
export function ValorExecutadoDialog({ open, onOpenChange, entries, onConfirm, isPending }: Props) {
  const today = format(new Date(), "yyyy-MM-dd");
  const [items, setItems] = useState<ValorExecutadoItem[]>([]);
          ...
          {items.map((item, idx) => (
            ...
            >
              <div className="flex items-start gap-3">
                <Checkbox checked={item.selected} onCheckedChange={() => toggleItem(idx)} className="mt-1" />
