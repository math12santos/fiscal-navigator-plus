...
export function FluxoCaixaTab() {
  ...
  const projetado = useMemo(() => entries.filter((e) => e.status === "previsto" || e.status === "confirmado"), [entries]);
  const realizado = useMemo(() => entries.filter((e) => e.status === "pago" || e.status === "recebido"), [entries]);

  const totaisRealizado = useMemo(() => {
    let entradas = 0, saidas = 0;
    for (const e of realizado) {
      const val = Number(e.valor_realizado ?? e.valor_previsto);
      if (e.tipo === "entrada") entradas += val; else saidas += val;
    }
