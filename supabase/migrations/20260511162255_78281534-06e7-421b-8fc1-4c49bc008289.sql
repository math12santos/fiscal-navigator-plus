...
BEGIN
  ...
  FOR v_sub_id IN SELECT public.get_all_subsidiary_ids(p_holding_id) LOOP
    v_subs := v_subs + 1;
    FOR m IN
    ...
    LOOP
      ...
      WHERE organization_id = v_sub_id AND module_key = m.module_key;

      IF v_existing IS NULL THEN
        INSERT INTO public.organization_modules (organization_id, module_key, enabled)
        VALUES (v_sub_id, m.module_key, m.enabled);
